...
macro_rules! xor {
    ($( $ident:ident => $key:literal),*) => {
        ...
        pub struct $ident;
        impl Filter for $ident {
            fn filter(&self, content: &mut [u8], _: u32) {
                for i in content{
                    *i^=$key;
                }
            }
...
fn asaproject_filter(content: &mut [u8], mut hash: u32) {
    hash &= 0x7fffffff;
    hash = (hash << 31) | hash;
...
// p20414 CloverGAME
xor!(v48419=>0x0A,v54776=>0xF8,v55899=>0xB9);

// p129 ALcot

null!(v16906, v18750, v29902);

// p1385 ALcot Honey Comb

null!(v21495);

pub struct v20833;
impl Filter for v20833 {
    fn filter(&self, _content: &mut [u8], hash: u32) {
        let _key = hash ^ 0xABCD9876;
    }
}
